...
/bin/rm -f ${T_DIR}/ApiGateway.Routing.Tests.jtl ${T_DIR}/ApiGateway.Routing.Tests.log  > /dev/null 2>&1
/bin/rm -f ${T_DIR}/CoreAPI.Websockets.Test.jtl ${T_DIR}/CoreAPI.Websockets.Test.log  > /dev/null 2>&1

./run.sh -Dlog_level.jmeter=DEBUG \
	-JTARGET_HOST=${TARGET_HOST} -JTARGET_PORT=${TARGET_PORT} \
	-JTARGET_PATH=${TARGET_PATH} -JTARGET_KEYWORD=${TARGET_KEYWORD} \
	-n -t ${T_DIR}/ApiGateway.Routing.Tests.jmx -l ${T_DIR}/ApiGateway.Routing.Tests.jtl -j ${T_DIR}/jmeter.log \
	-e -o ${R_DIR}
